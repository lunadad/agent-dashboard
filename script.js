...
function setupBriefingSearch() {
  ...
  input.addEventListener("input", () => {
    ...
    const filtered = !q
      ? state.briefings
      : state.briefings.filter((b) => `${b.time} ${b.headline}`.toLowerCase().includes(q));
    renderList("briefingList", filtered);
  });
}
...
(async function init() {
  ...
  setupThemeToggle();
  renderAgents();
  renderList("briefingList", state.briefings);
  renderList("noticeList", state.notices || []);
  renderMatrix("qualityBoard", state.quality || []);
  renderMatrix("sourceBoard", state.sources || []);
  renderMatrix("timeline", state.timeline);
  setupBriefingSearch();
})();
